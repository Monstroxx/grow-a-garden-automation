...
local AutomationConfig = {
    ...
    AutoBuySeeds = {
        Enabled = false,
        BuyAll = true, -- Default to buying all available seeds
        SelectedSeeds = {}, -- Will be populated automatically from SeedData
    },
    
    AutoBuyGear = {
        Enabled = false,
        BuyAll = true, -- Default to buying all available gear
        SelectedGear = {}, -- Will be populated automatically from GearData
    },
    
    AutoBuyEggs = {
        Enabled = false,
        BuyAll = true, -- Default to buying all available eggs
        SelectedEggs = {}, -- Will be populated automatically from PetEggData
    },
    ...
    AutoPlant = {
        Enabled = false,
        SelectedSeeds = {}, -- Will be populated automatically from SeedData
        SeedPriority = {}, -- Will be populated automatically from SeedData
        UseWateringCan = true,
        MaxPlantsPerType = 100, -- 100 = unlimited
        AutoReplant = true,
        OnlyPlantSelected = true,
        PlantingMode = "PlayerPosition", -- "PlayerPosition" or "RandomPosition"
    },
...
local CONFIG_FILE = "grow_garden_automation_config.txt"

-- Save configuration to file
local function SaveConfig()
    local success, error = pcall(function()
        local configData = HttpService:JSONEncode(AutomationConfig)
...
local function LoadConfig()
    local success, result = pcall(function()
        if isfile(CONFIG_FILE) then
            ...
            -- Merge loaded config with default config (deep merge)
...
-- Function to automatically populate game data (no longer auto-selects items)
local function PopulateGameData()
    -- Load saved configuration first
    LoadConfig()
    
    -- Note: We don't auto-populate selected items anymore
    -- Users need to manually configure their selections
    -- This ensures clean start and respects user choice
    
    print("🔄 Game data loading complete - ready for user configuration!")
    
    -- Update global config
    _G.AutomationSystem.Config = AutomationConfig
end

-- Populate game data automatically
PopulateGameData()

-- Function to refresh game data (call this when game updates)
_G.AutomationSystem.RefreshGameData = PopulateGameData

-- Auto-refresh game data every 5 minutes in case of updates
spawn(function()
    while true do
        wait(300) -- 5 minutes
        PopulateGameData()
...
function DataManager.GetBackpack()
    ...
    -- Convert InventoryData to simple name->quantity format (NO DEBUG OUTPUT)
    for uuid, itemInfo in pairs(inventoryData) do
        if typeof(itemInfo) == "table" and itemInfo.ItemType == "Seed" then
            local itemData = itemInfo.ItemData
            if itemData and itemData.ItemName and itemData.Quantity then
                local itemName = itemData.ItemName
                local seedName = itemName .. " Seed"
                local quantity = itemData.Quantity or 0
                
                backpack[seedName] = (backpack[seedName] or 0) + quantity
            end
        end
    end
    
    -- Also check legacy Backpack for compatibility
    local legacyBackpack = data.Backpack or {}
    for itemName, quantity in pairs(legacyBackpack) do
        if itemName:find("Seed") then
            backpack[itemName] = (backpack[itemName] or 0) + quantity
        end
    end
...
function FarmingManager.IsBackpackFull()
    local backpack = DataManager.GetBackpack()
    local itemCount = 0
    for _, amount in pairs(backpack) do
        itemCount = itemCount + amount
    end
    return itemCount >= 500 -- Assuming max backpack size is 500
end

function FarmingManager.GetPlantingPosition()
    local character = LocalPlayer.Character
    if not character or not character:FindFirstChild("HumanoidRootPart") then
        return nil
    end
    ...
    local playerPos = character.HumanoidRootPart.Position
    local mode = AutomationConfig.AutoPlant.PlantingMode or "PlayerPosition"
    
    if mode == "RandomPosition" then
        -- Find random position in Plant_Locations area
        local plantLocations = workspace:FindFirstChild("Plant_Locations")
        if plantLocations then
            local canPlantParts = {}
            for _, part in pairs(plantLocations:GetChildren()) do
                if part.Name == "Can_Plant" and part:IsA("Part") then
                    table.insert(canPlantParts, part)
                end
            end
            
            if #canPlantParts > 0 then
                -- Pick random Can_Plant part and generate position within it
                local randomPart = canPlantParts[math.random(#canPlantParts)]
                local partPos = randomPart.Position
                local partSize = randomPart.Size
                
                -- Generate random position within the part's bounds
                local randomX = partPos.X + (math.random() - 0.5) * partSize.X * 0.8
                local randomZ = partPos.Z + (math.random() - 0.5) * partSize.Z * 0.8
                
                return Vector3.new(randomX, partPos.Y + partSize.Y/2, randomZ)
            end
        end
        
        -- Fallback to player position if Plant_Locations not found
        return Vector3.new(playerPos.X, playerPos.Y, playerPos.Z)
    else
        -- Player position mode - exact player position
        return Vector3.new(playerPos.X, playerPos.Y, playerPos.Z)
    end
end

function FarmingManager.PlantSeed(seedType, position)
    ...
    -- Check MaxPlantsPerType limit (100 = unlimited)
    local maxPlants = AutomationConfig.AutoPlant.MaxPlantsPerType or 100
    if maxPlants < 100 then
        -- Count existing plants of this type
        local existingPlants = 0
        for _, obj in pairs(workspace:GetDescendants()) do
            if obj.Name:find(seedType) and obj:FindFirstChild("Plant") then
                existingPlants = existingPlants + 1
            end
        end
        
        if existingPlants >= maxPlants then
            return false -- Max plants reached
        end
    end
    ...
    local success, error = pcall(function()
        ...
        -- Check if seed tool is already equipped
        local equippedTool = character:FindFirstChildOfClass("Tool")
        local seedTool = nil
        
        if equippedTool then
            local itemType = equippedTool:GetAttribute("ITEM_TYPE") or equippedTool:GetAttribute("b")
            if itemType == "n" and equippedTool.Name:find(seedType) then
                seedTool = equippedTool
                -- Tool already equipped, proceed to plant
            end
        end
        
        -- If not equipped, find and equip seed tool from backpack
        if not seedTool then
            for _, tool in pairs(LocalPlayer.Backpack:GetChildren()) do
                if tool:IsA("Tool") then
                    local itemType = tool:GetAttribute("ITEM_TYPE") or tool:GetAttribute("b")
                    if itemType == "n" and tool.Name:find(seedType) then
                        seedTool = tool
                        break
                    end
                end
            ...
            if seedTool then
                ...
                for i = 1, 20 do -- Wait up to 2 seconds
                    ...
                    if newEquippedTool and newEquippedTool == seedTool then
                        equipped = true
                        break
                    end
                    wait(0.1)
                ...
                if not equipped then
                    print("❌ Failed to equip seed tool:", seedName)
                    return false
                end
            else
                print("❌ Seed tool not found in backpack:", seedName)
                return false
            end
        end
        
        -- Now we have a equipped seed tool - plant it!
        if seedTool then
            local PlantRemote = ReplicatedStorage.GameEvents:FindFirstChild("Plant_RE")
            if PlantRemote then
                PlantRemote:FireServer(position, seedType)
                wait(0.5) -- Wait for planting to complete
            end
            
            -- Auto-water if enabled
            if AutomationConfig.AutoPlant.UseWateringCan then
                local WaterRemote = ReplicatedStorage.GameEvents:FindFirstChild("Water_RE")
                if WaterRemote then
                    WaterRemote:FireServer(position)
                end
            end
        end
    end)
    ...
    if success then
        ...
        print("❌ Planting failed:", error)
    end
    
    return success
end

function FarmingManager.SellInventory()
    if not AutomationConfig or not AutomationConfig.AutoCollect or not AutomationConfig.AutoCollect.AutoSell then
        return false
    end
    ...
    local success, error = pcall(function()
        ...
        -- Use the exact sell location from game data
        local sellLocation = workspace.Tutorial_Points and workspace.Tutorial_Points.Tutorial_Point_2
        if sellLocation then
            -- Teleport to sell NPC location (same as game's "Sell" teleport button)
            character.HumanoidRootPart.CFrame = sellLocation.CFrame + Vector3.new(5, 0, 0)
            wait(1) -- Wait for teleport
            
            -- Use Sell_Inventory remote
            local SellRemote = ReplicatedStorage.GameEvents:FindFirstChild("Sell_Inventory")
            if SellRemote then
                SellRemote:FireServer()
                wait(1) -- Wait for sell to complete
            end
            
            -- Teleport back to original position
            character.HumanoidRootPart.CFrame = originalCFrame
            wait(0.5) -- Wait for return teleport
            
            print("💰 Sold inventory and returned to farming")
        else
            -- Fallback: Try sell remote without teleport
            local SellRemote = ReplicatedStorage.GameEvents:FindFirstChild("Sell_Inventory")
            if SellRemote then
                SellRemote:FireServer()
            end
            print("💰 Sold inventory (no teleport)")
        end
    end)
...
function FarmingManager.CollectPlants()
    -- Check if backpack is full first
    if FarmingManager.IsBackpackFull() then
        -- Auto-sell if enabled
        if AutomationConfig.AutoCollect.AutoSell then
            FarmingManager.SellInventory()
            wait(2) -- Wait for sell to complete
        else
            return
        end
    end
    ...
    for _, plant in pairs(plants) do
        ...
        local success, error = pcall(function()
            ...
            if character and character:FindFirstChild("HumanoidRootPart") then
                ...
                -- Use proximity prompt (most common harvest method)
...
function ShopManager.BuySeeds()
    local success, error = pcall(function()
        if not AutomationConfig or not AutomationConfig.AutoBuySeeds or not AutomationConfig.AutoBuySeeds.Enabled then 
            -- BuySeeds: Not enabled or config missing
            return 
        end
    ...
    for _, seedType in pairs(seedsToBuy) do
        ...
        for attempt = 1, maxAttempts do
            ...
            if success then
                ...
                -- Seed purchase completed
            else
                -- Seed stock depleted
                if purchaseCount > 0 then
                    -- Seeds purchased successfully
                else
                    -- Failed to buy seed
                end
                break
...
function ShopManager.BuyGear()
    if not AutomationConfig or not AutomationConfig.AutoBuyGear or not AutomationConfig.AutoBuyGear.Enabled then 
        -- BuyGear: Not enabled or config missing
        return 
    end
    ...
    for _, gearType in pairs(gearToBuy) do
        ...
        for attempt = 1, maxAttempts do
            ...
            if success then
                purchaseCount = purchaseCount + 1
                -- Gear purchase completed
            else
                -- Gear stock depleted
                if purchaseCount > 0 then
                    -- Gear purchased successfully
                else
                    -- Failed to buy gear
                end
                break
...
function ShopManager.BuyEggs()
    if not AutomationConfig or not AutomationConfig.AutoBuyEggs or not AutomationConfig.AutoBuyEggs.Enabled then 
        -- BuyEggs: Not enabled or config missing
        return 
    end
    ...
    for _, eggType in pairs(eggsToBuy) do
        ...
        for attempt = 1, maxAttempts do
            ...
            if success then
                purchaseCount = purchaseCount + 1
                -- Egg purchase completed
            else
                -- Egg stock depleted
                if purchaseCount > 0 then
                    -- Eggs purchased successfully
                else
                    -- Failed to buy egg
                end
                break
...
local function MainLoop()
    ...
    while true do
        if AutomationConfig and AutomationConfig.Enabled then
            -- Shopping (continuous when enabled) - Check both master and individual enables
            if AutomationConfig.AutoBuySeeds and AutomationConfig.AutoBuySeeds.Enabled and 
               currentTime - lastTasks.buySeeds >= 5 then -- Check every 5 seconds for restock
            ...
            if AutomationConfig.AutoBuyGear and AutomationConfig.AutoBuyGear.Enabled and
               currentTime - lastTasks.buyGear >= 5 then -- Check every 5 seconds for restock
            ...
            if AutomationConfig.AutoBuyEggs and AutomationConfig.AutoBuyEggs.Enabled and
               currentTime - lastTasks.buyEggs >= 5 then -- Check every 5 seconds for restock
            ...
            if AutomationConfig.AutoPlant and AutomationConfig.AutoPlant.Enabled then
                pcall(function()
                    -- Spam planting - no interval check
                    for _, seedType in pairs(AutomationConfig.AutoPlant.SelectedSeeds or {}) do
                        local position = FarmingManager.GetPlantingPosition()
                        if position then
                            if FarmingManager.PlantSeed(seedType, position) then
                                -- Continue spamming - don't break
...
local function GetAvailableEggs()
    ...
    if PetEggData then
        for eggName, eggData in pairs(PetEggData) do
            table.insert(eggs, {
                name = eggName,
                rarity = eggData.EggRarity or "Common",
                price = eggData.Price,
                id = eggData.PurchaseID
            })
        end
...
function CreateAutoBuySection(parent)
    ...
    CreateToggle(seedsContent, "Buy All Seeds", "Buy all available seeds or only selected ones", AutomationConfig.AutoBuySeeds, "BuyAll", 2)
    ...
    CreateToggle(gearContent, "Buy All Gear", "Buy all available gear or only selected ones", AutomationConfig.AutoBuyGear, "BuyAll", 2)
    ...
    CreateToggle(eggsContent, "Buy All Eggs", "Buy all available eggs or only selected ones", AutomationConfig.AutoBuyEggs, "BuyAll", 2)
...
function CreateFarmingSection(parent)
    ...
    modeButton.Size = UDim2.new(0.55, 0, 0, 30)
    modeButton.Position = UDim2.new(0.4, 5, 0, 10)
    modeButton.BackgroundColor3 = Color3.fromRGB(28, 28, 30)
    modeButton.Text = AutomationConfig.AutoPlant.PlantingMode or "PlayerPosition"
    ...
    modeButton.MouseButton1Click:Connect(function()
        print("🔄 Planting mode button clicked!")
        if AutomationConfig.AutoPlant.PlantingMode == "PlayerPosition" then
            AutomationConfig.AutoPlant.PlantingMode = "RandomPosition"
            print("📍 Switched to RandomPosition")
        else
            AutomationConfig.AutoPlant.PlantingMode = "PlayerPosition"
            print("📍 Switched to PlayerPosition")
        end
        modeButton.Text = AutomationConfig.AutoPlant.PlantingMode
        print("💾 Saving config...")
        SaveConfig()
    end)
    
    CreateSlider(plantContent, "Max Plants Per Type", "Maximum plants per type (100 = unlimited)", AutomationConfig.AutoPlant, "MaxPlantsPerType", 1, 100, 5)
    
    -- Seed selector for planting
    CreateItemSelector(plantContent, "Seeds to Plant", "Choose which seeds to plant", GameItems.Seeds, AutomationConfig.AutoPlant, "SelectedSeeds", 8)
    ...
    CreateToggle(collectContent, "Auto Sell", "Automatically sell inventory when full", AutomationConfig.AutoCollect, "AutoSell", 3)
    CreateSlider(collectContent, "Collect Interval", "Seconds between collections", AutomationConfig.AutoCollect, "CollectInterval", 0.5, 5, 4)
    CreateSlider(collectContent, "Collect Radius", "Collection radius in studs", AutomationConfig.AutoCollect, "CollectRadius", 50, 500, 5)
end
...
function CreatePetSection(parent)
    ...
    CreateToggle(petContent, "Auto Feed", "Automatically feed pets", AutomationConfig.PetManagement, "AutoFeed", 2)
    CreateToggle(petContent, "Auto Hatch Eggs", "Automatically hatch eggs", AutomationConfig.PetManagement, "AutoHatchEggs", 3)
