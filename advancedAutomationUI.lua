...
function CreateEventsSection(parent)
    ...
    -- Daily Events Card
    local dailyCard = CreateCard(scroll, "📅 Daily Events", "Automatic daily activities", 0)
    
    CreateToggle(dailyCard, "Enable Auto Events", "Enable all event automation", AutomationConfig.AutoEvents, "Enabled", 0)
    CreateToggle(dailyCard, "Daily Quests", "Auto claim daily quest rewards", AutomationConfig.AutoEvents, "DailyQuests", 1)
    CreateToggle(dailyCard, "Infinite Pack Claims", "Auto claim infinite pack rewards", AutomationConfig.AutoEvents, "InfinitePacks", 2)
    CreateToggle(dailyCard, "Auto Claim Rewards", "Automatically claim all available rewards", AutomationConfig.AutoEvents, "AutoClaim", 3)
    
    -- Special Events Card
    local specialCard = CreateCard(scroll, "🌙 Special Events", "Participate in special events", 1)
    
    CreateToggle(specialCard, "Summer Harvest", "Auto participate in summer harvest events", AutomationConfig.AutoEvents, "SummerHarvest", 0)
    CreateToggle(specialCard, "Night Events", "Auto participate in night events", AutomationConfig.AutoEvents, "NightEvents", 1)
    CreateToggle(specialCard, "Blood Moon", "Auto participate in blood moon events", AutomationConfig.AutoEvents, "BloodMoon", 2)
    CreateToggle(specialCard, "Bee Swarm", "Auto participate in bee swarm events", AutomationConfig.AutoEvents, "BeeSwarm", 3)
    
    -- Dino Events Card
    local dinoCard = CreateCard(scroll, "🦕 Dino Events", "Dinosaur-themed event automation", 2)
    
    CreateToggle(dinoCard, "Dino Machine", "Auto use dino machine", AutomationConfig.AutoEvents, "DinoMachine", 0)
    CreateToggle(dinoCard, "Dino Quests", "Auto complete dino quests", AutomationConfig.AutoEvents, "DinoQuests", 1)
    CreateToggle(dinoCard, "Dino Rewards", "Auto claim dino rewards", AutomationConfig.AutoEvents, "DinoRewards", 2)
    
    -- Event Timing Card
    local timingCard = CreateCard(scroll, "⏰ Event Timing", "Configure event checking intervals", 3)
    
    CreateSlider(timingCard, "Check Interval", "Seconds between event checks", AutomationConfig.AutoEvents, "CheckInterval", 10, 300, 0)
    CreateSlider(timingCard, "Retry Attempts", "Max attempts before giving up", AutomationConfig.AutoEvents, "MaxRetries", 1, 10, 1)
end
...
-- Config Synchronization Functions
local function SyncConfigWithBackend()
    -- Ensure _G.AutomationSystem exists
    if not _G.AutomationSystem then
        _G.AutomationSystem = {}
    end
    
    -- Sync UI config to backend
    _G.AutomationSystem.Config = AutomationConfig
    
    print("🔄 Config synced to backend")
end

local function LoadConfigFromBackend()
    -- Load config from backend if it exists
    if _G.AutomationSystem and _G.AutomationSystem.Config then
        -- Deep merge backend config with UI config
        ...
        deepMerge(AutomationConfig, _G.AutomationSystem.Config)
